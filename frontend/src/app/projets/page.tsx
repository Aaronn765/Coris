...
export default function ProjetsPage() {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const blob = await apiService.exportProjets({
        ...query,
      ...
      anchor.href = url;
      anchor.download = "projets-export.xlsx";
      anchor.rel = "noopener";
  ...
  return (
    <div className="animate-fade-up space-y-4 sm:space-y-6">
      <header className="flex flex-col justify-between gap-4 border-b border-slate-200/80 pb-4 sm:flex-row sm:items-end sm:pb-5"><div><h1 className="text-2xl font-bold tracking-tight text-slate-800 sm:text-3xl">Projets</h1><p className="mt-1 text-sm text-slate-500">Suivi des projets DSI et de leurs étapes</p></div><div className="grid gap-2 sm:flex sm:flex-wrap"><Button onClick={handleExport} disabled={exporting} variant="outline" className="h-10 w-full rounded-xl border-slate-200 text-coris-blue hover:border-coris-blue hover:bg-coris-blue-soft sm:w-auto">{exporting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />} Exporter Excel</Button><Link href="/projets/nouveau"><Button className="w-full rounded-xl bg-coris-red text-white shadow-lg shadow-red-900/15 hover:bg-[#c91428] sm:w-auto"><Plus className="mr-2 h-4 w-4" /> Nouveau projet</Button></Link></div></header>
